...
- [x] **T1. Зафиксировать постоянные правила тестирования.**
  Правила находятся в `knowledge-base.md` и не зависят от
  устройства других кодовых баз.
- [x] **T2. Сгруппировать тесты по границе продукта.** HTTP API-проверки,
  включая PostgreSQL functional suite, находятся в `src/tests/api/`; проверки
  HTML views, сервисов и фоновых задач — в `src/tests/views/`,
  `src/tests/services/` и `src/tests/tasks/`. Functional DB-fixture включается
  явно, поэтому unit API-тесты не требуют `TEST_DB_NAME`.
