...
class AuthCoreAPIController(BaseAuthAPIController):
    ...
    async def reset_password(
        self, data: ResetPasswordRequest, request: Request, settings: AppSettings
    ) -> OKResponse:
        ...
        if user is not None and user.is_active:
            ...
            await send_reset_password_email(
                ...
                token,
                settings=settings,
                mailer=cast(Any, request.app).providers.mailer,
            )
...
class AuthInviteAPIController(BaseAuthAPIController):
    ...
    async def get_invites(
        self, request: Request, limit: int = 10, offset: int = 0
...
class AuthProfileAPIController(BaseAuthAPIController):
    ...
    async def update_me(self, data: ProfileUpdateRequest, request: AppRequest) -> UserResponse:
        ...
        if update_data:
            async with _get_uow(request) as uow:
                user_repository = UserRepository(uow.session)
                user = await user_repository.first(id=request.user.id)
                if user is None:
                ...
                await user_repository.update(user, **update_data)
                uow.mark_for_commit()
...
class AuthAccessTokenAPIController(BaseAuthAPIController):
    ...
    async def delete_access_token(
        self, token_id: int, request: Request, current_user: User
    ) -> None:
