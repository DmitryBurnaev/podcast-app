...
class BaseAuthBackend(abc.ABC):
    ...
    async def create_cookie_session(self, user: User, *, expires_in: int) -> tuple[str, datetime]:
        ...
        now = utcnow()
        async with SASessionUOW() as uow:
            await UserSessionRepository(uow.session, scope=OwnerScope(user.id)).create(
    ...
    async def register_user_ip(self, user: User | None = None) -> None:
        ...
        try:
            async with SASessionUOW() as uow:
                repository = UserIPRepository(uow.session, scope=OwnerScope(_user.id))
...
class APIAuthBackend(BaseAuthBackend):
    ...
    async def create_user_session(self, user: User) -> TokenCollection:
        ...
        async with SASessionUOW() as uow:
            session_repo = UserSessionRepository(uow.session, scope=OwnerScope(user.id))
